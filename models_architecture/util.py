...
class MetricsF1(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, batch, logs=None):
        if logs is None:
            logs = {}
        # X_val, y_truth = self.validation_data[0], self.validation_data[1].reshape(-1)
        # y_predict = np.asarray(self.model.predict(X_val))
        #
        # y_predict = np.argmax(y_predict, axis=1)
        #
        # print(classification_report(y_truth, y_predict, labels=[0, 1, 2],
        #                             target_names=['CLEAN', 'OFFENSIVE_BUT_NOT_HATE', 'HATE']))